...
import { appHref, getAppPath } from "../../app/routing";
...
export function SiteHeader() {
  ...
  return (
    <header className="site-header">
      <a className="brand" href={appHref("/")} aria-label="PixelLock Image Converter home">
        ...
        <details className="converter-menu">
          ...
          <nav aria-label="All converters">
            ...
            {converterCategories.map((category) => {
              const href = category.id === "image" ? appHref("/") : appHref(category.path);
            ...
            <a href={appHref("/tools/gif-compressor")} aria-current={pathname === "/tools/gif-compressor" ? "page" : undefined}><MenuLinkContent icon="gif" label="GIF Compressor" note="Reduce animated GIF size" /></a>
            <span className="menu-label">PDF tools</span>
            <a href={appHref("/pdf-tools/compress")} aria-current={pathname === "/pdf-tools/compress" ? "page" : undefined}><MenuLinkContent icon="compress" label="Compress PDF" note="Reduce PDF file size" /></a>
            <a href={appHref("/pdf-tools/organize")} aria-current={pathname === "/pdf-tools/organize" ? "page" : undefined}><MenuLinkContent icon="organize" label="Organize PDF" note="Reorder and rotate pages" /></a>
            <a href={appHref("/pdf-tools/split")} aria-current={pathname === "/pdf-tools/split" ? "page" : undefined}><MenuLinkContent icon="split" label="Split PDF" note="Separate pages into files" /></a>
          </nav>
        </details>
        <ThemeToggle />
      </div>
